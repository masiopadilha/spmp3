inherited FrmTelaCadManutProgFamEquipClonagem: TFrmTelaCadManutProgFamEquipClonagem
  Caption = 'Criar Manuten'#231#245'es nos Equipamentos da Fam'#237'lia'
  ClientHeight = 514
  ClientWidth = 953
  ...
  object qryEquipamentos: TFDQuery
    ...
    SQL.Strings = (
      ...
      '                              INNER JOIN `manutprogequipamento`'
        ...
        '                                  ON (e2.`CODIGO` = `manutprogeq' +
        'uipamento`.`CODEQUIPAMENTO`) AND (`manutprogequipamento`.`CODEMP' +
        'RESA` = e2.`CODEMPRESA`)'
      
        #9#9#9'  WHERE (`manutprogequipamento`.`CODMANUTPROGFAMEQUIP` = :cod' +
        'manutprogfamequip'
      
        #9#9#9'      AND `manutprogequipamento`.`CODEMPRESA` = e1.`CODEMPRES' +
        'A`)'
