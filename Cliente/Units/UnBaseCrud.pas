...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, UnBaseHeranca, Data.DB, Vcl.StdCtrls,
  Vcl.Grids, Vcl.DBGrids, JvExDBGrids, JvDBGrid, Vcl.WinXCtrls, Vcl.ComCtrls,
  Vcl.ExtCtrls, Vcl.Buttons, System.ImageList, Vcl.ImgList, FireDAC.Comp.Client, FireDAC.Stan.Param,
  FireDAC.Stan.Error, Vcl.DBCtrls, Vcl.Menus, System.Actions, Vcl.ActnList,
  Vcl.ExtActns;

type
  TfrmBaseCrud = class(TfrmBaseHeranca)
    ...
    lblCadastro: TLabel;
    lblDBCadastro: TDBText;
    lblCadastroPor: TLabel;
    lblDBCadastroPor: TDBText;
    lblAlteracao: TLabel;
    LblDBAlteracao: TDBText;
    lblAlteracaoPor: TLabel;
    lblDBAlteracaoPor: TDBText;
...
uses UnDM, UnTelaCadMonitMedicoes, UnTelaConsulta, UnDmRelatorios;

procedure TfrmBaseCrud.MostrarValorHint;
var
c:Integer;
begin
...
procedure TfrmBaseCrud.ControleBotoes(Indice: SmallInt);
begin
  case Indice of
    0://Vazio
      begin
...
procedure TfrmBaseCrud.ChbAtivoConsClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.chbVisivelConsClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.Completo1Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.Consultar;
var
  LMensagem: PChar;
begin
  inherited;
...
procedure TfrmBaseCrud.ActionList1Change(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.btnAjudaClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.btnConsultarClick(Sender: TObject);
var
  c:SmallInt;
begin
  inherited;
  ...
  Finally
    ...
    lblDBCadastroPor.ShowHint := True;
    if lblDBCadastroPor.DataField <> '' then
      if lblDBCadastroPor.DataSource.DataSet.FieldByName('USUARIOCAD').AsString <> '' then
          lblDBCadastroPor.Hint := lblDBCadastroPor.DataSource.DataSet.FieldByName('USUARIOCAD').AsString
      else
      if lblDBCadastroPor.DataSource.DataSet.IsEmpty = False then
        lblDBCadastroPor.Hint := 'Nome do usuário não localizado.'
      else
        lblDBCadastroPor.ShowHint := False;

    lblDBAlteracaoPor.ShowHint := True;
    if lblDBAlteracaoPor.DataField <> '' then
      if lblDBAlteracaoPor.DataSource.DataSet.FieldByName('USUARIOALT').AsString <> '' then
          lblDBAlteracaoPor.Hint := lblDBAlteracaoPor.DataSource.DataSet.FieldByName('USUARIOALT').AsString
      else
      if lblDBAlteracaoPor.DataSource.DataSet.IsEmpty = False then
        lblDBAlteracaoPor.Hint := 'Nome do usuário não localizado.'
      else
        lblDBAlteracaoPor.ShowHint := False;
...
procedure TfrmBaseCrud.btnImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.btnNovoClick(Sender: TObject);
var
  c:SmallInt;
begin
  inherited;
...
procedure TfrmBaseCrud.btnSalvarClick(Sender: TObject);
begin
  inherited;
  PAuxiliares.Font.Color := clBlue;
  PAuxiliares.Caption    := '';
...
begin
  inherited;
  DM.FModulo := '';
  if Assigned(DM.FDataSetParam) then
    DM.FDataSetParam.Free;
end;
...
if Key = 27 then
  begin
    if Application.MessageBox('Deseja realmente sair?', 'SPMP4', MB_YESNO + MB_ICONQUESTION) = IDYes then
      Close;
  end;
end;
...
If Key = #13 Then
  ...
  Except
    Begin
      Application.MessageBox('Não foi possível identificar esse valor, por favor verifique o valor informado.','SPMP4',MB_OK + MB_ICONERROR);
    End;
  End;
end;

procedure TfrmBaseCrud.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  inherited;
  MostrarValorHint;
...
procedure TfrmBaseCrud.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseCrud.Individual1Click(Sender: TObject);
begin
  inherited;
  if DM.FDataSetParam.IsEmpty = True then Exit;
...
procedure TfrmBaseCrud.JvDBGridListaDblClick(Sender: TObject);
var
c:SmallInt;
begin
  inherited;
...
//  if EdtConsulta.Focused = True then Exit;
  ...
  lblDBCadastro.ShowHint := True;
  if lblDBCadastro.DataField <> '' then
    if lblDBCadastro.DataSource.DataSet.FieldByName('USUARIOCAD').AsString <> '' then
        lblDBCadastro.Hint := lblDBCadastro.DataSource.DataSet.FieldByName('USUARIOCAD').AsString
    else
    if lblDBCadastro.DataSource.DataSet.IsEmpty = False then
      lblDBCadastro.Hint := 'Nome do usuário não localizado.'
    else
      lblDBCadastro.ShowHint := False;

  LblDBAlteracao.ShowHint := True;
  if LblDBAlteracao.DataField <> '' then
    if LblDBAlteracao.DataSource.DataSet.FieldByName('USUARIOALT').AsString <> '' then
        LblDBAlteracao.Hint := LblDBAlteracao.DataSource.DataSet.FieldByName('USUARIOALT').AsString
    else
    if LblDBAlteracao.DataSource.DataSet.IsEmpty = False then
      LblDBAlteracao.Hint := 'Nome do usuário não localizado.'
    else
      LblDBAlteracao.ShowHint := False;
...
procedure TfrmBaseCrud.SearchBoxConsultaInvokeSearch(Sender: TObject);
begin
  inherited;
  Consultar;
end;

procedure TfrmBaseCrud.SearchBoxConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
