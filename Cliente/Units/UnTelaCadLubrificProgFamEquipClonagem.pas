unit UnTelaCadLubrificProgFamEquipClonagem;
...
uses
  ...
  Vcl.ExtCtrls, Vcl.Imaging.pngimage, Data.DB, Vcl.Buttons, Vcl.Grids,
  Vcl.DBGrids, UnDM, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,
  FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf,
  FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client;

type
  TFrmTelaCadLubrificProgFamEquipClonagem = class(TFrmTelaPaiOKCancel)
...
var
  FrmTelaCadLubrificProgFamEquipClonagem: TFrmTelaCadLubrificProgFamEquipClonagem;
...
procedure TFrmTelaCadLubrificProgFamEquipClonagem.BitBtn2Click(Sender: TObject);
var
  I: SmallInt;
  LInsp: String;
  LTexto: PChar;
...
begin
  ...
  try
    ...
    begin
      LTexto := PChar('Escolha o(s) equipamento(s) para cadastrar o plano de lubrificação.');
    ...
    LTexto := PChar('Deseja realmente criar o plano de lubrificação para o(s) equipamento(s) selecionado(s)?' + #13 + LInsp);
    ...
    begin
      ...
      begin
        ...
        DM.qryAuxiliar2.FieldByName('CODLUBRIFICPROGFAMEQUIP').AsString := DM.qryLubrificProgFamEquipCODIGO.AsString;
        DM.qryAuxiliar2.FieldByName('MATRICULA').AsString            := EdtMatricula.Text;
        DM.qryAuxiliar2.FieldByName('DESCRICAO').AsString            := DM.qryLubrificProgFamEquipDESCRICAO.AsString;
        DM.qryAuxiliar2.FieldByName('CRITICIDADE').AsString          := cbCriticidade.Text;
        DM.qryAuxiliar2.FieldByName('FREQUENCIA1').AsString          := DM.qryLubrificProgFamEquipFREQUENCIA.AsString;
    ...
    on E: Exception do
    begin
      DM.GravaLog('Falha ao clonar o plano de lubrificação.' + Screen.ActiveForm.Name + ' ', E.ClassName, E.Message);
      Application.MessageBox('Falha ao clonar o plano de lubrificação!, entre em contato com o suporte.', 'SPMP3', MB_OK + MB_ICONERROR);
    end;
  end;
end;

procedure TFrmTelaCadLubrificProgFamEquipClonagem.BtnFuncionarioClick(
  Sender: TObject);
begin
  inherited;
...
procedure TFrmTelaCadLubrificProgFamEquipClonagem.chbTodosClick(
  Sender: TObject);
var
  i: Integer;
begin
...
procedure TFrmTelaCadLubrificProgFamEquipClonagem.EdtMatriculaKeyPress(
...
procedure TFrmTelaCadLubrificProgFamEquipClonagem.FormCreate(Sender: TObject);
begin
  inherited;
  qryEquipamentos.Close;
  qryEquipamentos.Params[0].AsString := DM.qryLubrificProgFamEquipCODIGO.AsString;
  qryEquipamentos.Params[1].AsString := DM.FCodEmpresa;
  qryEquipamentos.Params[2].AsString := DM.qryLubrificProgFamEquipCODFAMILIAEQUIP.AsString;
  qryEquipamentos.Open;

  gbEquipamentosFamilia.Caption := 'Equipamentos da Família: ' + DM.qryLubrificProgFamEquipFAMILIAEQUIPAMENTO.AsString;
end;
